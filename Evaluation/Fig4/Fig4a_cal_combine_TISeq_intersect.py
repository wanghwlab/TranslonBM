...
BASE_OUTPUT_DIR = "/home/tangyuewen/ORF_benchmark/final_ORFs_2026.1/plots_combine/TISeq_score_intersect_untrim/"
            ...
            for sample, tools_data in samples_data.items():
                ...
                for filter_type in ['ATG', 'NTG']:
                    ...
                    for tool_a, tool_b in itertools.combinations(tools, 2):
                        ...
                        dict_a = {orf_id: tis for orf_id, tis in orfs_a}
                        dict_b = {orf_id: tis for orf_id, tis in orfs_b}

                        common_ids = set(dict_a.keys()) & set(dict_b.keys())

                        orfs_combined_deduplicated = [(orf_id, dict_a[orf_id]) for orf_id in common_ids]
