...
duplicate_ORF <- possibly(function(block, tid_withseq, ORF_gcoor_reformat) {
    ORF_blocks <- read_tsv(
        block,
        col_names = F,
    ...
    rawfile_sequence <- read_tsv(
        ...
        mutate(start_codon = stringr::str_sub(ORF_sequence_correct, 1, 3))
    ...
    ORF_blocks <- ORF_blocks |>
        inner_join(
            ...
            by = c(
                ...
                "ORF_tstart_1base" = "ORF_tstart_1base",
                "ORF_tstop_1base" = "ORF_tstop_1base"
            )
        ) |>
        dplyr::mutate(
