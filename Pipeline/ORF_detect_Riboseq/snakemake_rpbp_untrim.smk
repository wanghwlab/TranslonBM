...
#################################
OUT_DIR = "/home/tangyuewen/ORF_benchmark/rerun_2025.9/ORFdetect_untrim/rpbp"
...
rule all:
  input:
    ...
    expand('rpbp_chrN/orf_pred_default/{sample}_{mpsf}/without-rrna-mapping/{sample}_{mpsf}.bam', sample=SAMPLES, mpsf=MAPPING_SOFTWARE),
...
#  output:
#    'rpbp_chrN/orf_pred_default/{sample}_{mpsf}/without-rrna-mapping/{sample}_{mpsf}.bam'
#  run:
...
rule create_symlink_default:
    input:
        bam_file = os.path.join(BAM_DIR, '{sample}_{mpsf}.bam')
    output:
        bam_link = 'rpbp_chrN/orf_pred_default/{sample}_{mpsf}/without-rrna-mapping/{sample}_{mpsf}.bam',
        bai_link = 'rpbp_chrN/orf_pred_default/{sample}_{mpsf}/without-rrna-mapping/{sample}_{mpsf}.bam.bai'
    params:
        source_bai = os.path.join(BAM_DIR, '{sample}_{mpsf}.bam.bai')
    run:
        import os
        import subprocess

        if os.path.lexists(output.bam_link): os.remove(output.bam_link)
        os.symlink(os.path.abspath(input.bam_file), output.bam_link)

        if os.path.exists(params.source_bai):
            if os.path.lexists(output.bai_link): os.remove(output.bai_link)
            os.symlink(os.path.abspath(params.source_bai), output.bai_link)
            print(f"Symlinked existing index: {params.source_bai}")
        else:
            print(f"Source index not found. Generating index for {output.bam_link}...")
            subprocess.check_call(f"samtools index {output.bam_link}", shell=True)

rule run_rpbp_chrN_default:
  input:
    bam = 'rpbp_chrN/orf_pred_default/{sample}_{mpsf}/without-rrna-mapping/{sample}_{mpsf}.bam',
