.libPaths(c('/home/tangyuewen/R/4.3','/home/tangyuewen/miniconda3/envs/r_deseq2/lib/R/library'))
library(tidyverse)
...
## need improve the absoluted path

output_func <- function(df, output_path) {
    readr::write_tsv(df, output_path, col_names = T, quote = "none")
}

## first format raw result

preprocessing <- possibly(function(addchr_bed, ORF_sequence_tsv) {
...
## snakemake will always ensure input are valid
...
ORF_processed <- preprocessing(
    addchr_bed = args[1],
    ORF_sequence_tsv = args[2]
)

format_ORF(
    ORF_blocks = ORF_processed,
    formatted_ORF = args[3]
)
